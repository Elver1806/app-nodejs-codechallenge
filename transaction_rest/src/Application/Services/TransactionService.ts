...
export class TransactionService {
  ...
  async getTransaction(idTransaction: string): Promise<Transaction> {
    ...
    this.logger.log(`Tarnsaction found => ${JSON.stringify(transaction)}`);
    if (transaction) {
  ...
  async createTransaction(
    ...
    const producerRecord = {
      ...
      messages: [
        {
          key: this.configService.get<string>('KAFKA_CLIENT_ID'),
    ...
    await this.producerKafkaDomainService.sendMessage(producerRecord);
    this.logger.log(
      `Result Send Transaction=> ${JSON.stringify(producerRecord)}`,
    );
    return resultSave;
  }
